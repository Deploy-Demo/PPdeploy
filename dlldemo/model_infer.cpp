	...
	int target_width, int target_height, const char* shape_range_info_path)
{
	...
	std::string cfg_file = std::string(model_dir) + "\\";
...
// 新增二次封装接口api
extern "C" __declspec(dllexport) ModelWrapper * ModelObjInit(const char* model_type, const char* model_dir, int gpu_id, bool use_trt, char* paddlex_model_type,
